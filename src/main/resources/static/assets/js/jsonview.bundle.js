    ...
    var getSizeString = function getSizeString(node) {
      ...
      if (node.type === 'object') {
			...
			if (node.key=='attributes') {
				key = '['+Object.entries(node.value).length+']';//+Object.entries(node.value)[0][1]+']';
			}
			else {
				var desc = '';
				if (node.value.description) desc = node.value.description;
				else {
					if (node.value['attributes'])
						desc = Object.entries(node.value['attributes'])[0][1];
				}
				key = ': <b>'+(node.value.code?node.value.code:'root') + (desc.length>0?' - '+desc:'')+'</b>';
			}
			return key;
	  }
      return null;
